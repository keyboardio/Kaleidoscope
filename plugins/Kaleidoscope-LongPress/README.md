...
To define the keys that should behave differently on long-press use include
a definition like the following:

```c++
LONGPRESS(
    kaleidoscope::plugin::LongPressKey(        KeyAddr(1, 1),     Key_Q),         // Long-press the key on QWERTY position of “q” to enter a “q” on all layers
    kaleidoscope::plugin::LongPressKey(QWERTY, KeyAddr(2, 4),     LCTRL(Key_C)),  // Long-press the key below the index finger to enter “Ctrl-C“ on the QWERTY layer
    kaleidoscope::plugin::LongPressKey(        Key_T,             RALT(Key_T)),   // Long-press “t” to enter a “þ” on all layers
)
```
...
Another thing that can be seen in the example above is that long presses on
`KeyAddr` can be restricted to a single layer (the second one is restricted to
the QWERTY layer). This parameter is optional and if it is missing the entry
applies to all layers.
Long presses on `Key` always apply to all layers.
