...
// By defining macro KALEIDOSCOPE_REMOTE_CALL_DISABLED at the top of
// the sketch file, the overall remote call infrastructure can be disabled.
//
#define KALEIDOSCOPE_REMOTE_CALL_DISABLED

// #define KALEIDOSCOPE_ENABLE_MACRO_NEWLINE_SUBSTITUTION
...
KALEIDOSCOPE_REMOTE_CALL(
  ...
  KRC_ROOT_PACKAGE(ProcedureParentPackage,
                   KRC_FUNCTION(setAllLEDs,
                                KRC_ARGUMENTS(
                                  (uint8_t, red, KRC_DESCRIPTION("red portion")),
                                  (uint8_t, green, KRC_DESCRIPTION("red portion")),
                                  (uint8_t, blue, KRC_DESCRIPTION("blue portion"))
                                )
                                KRC_RESULTS(
                                  (uint8_t, res, KRC_DESCRIPTION("res desc."))
                                )
                                KRC_FUNCTION_BODY(
  ...
  KRC_ROOT_PACKAGE(PackageLevel1,
                   KRC_DESCRIPTION("PackageLevel1 description")
                   KRC_UPDATE_THROUGH_GLOBAL_FUNCTION(::updateMyPlugin)

                   // A package that exports members of a plugin.
                   //
                   KRC_PLUGIN(MyPlugin, kaleidoscope::plugin::MyPlugin,
                              ...
                              KRC_MEMBER(a, a_,
                                  ...
                                  // Will inherit update function from PackageLevel1.
                                        )
                              KRC_MEMBER(b, b_,
                   ...
                   // A package that only deals with global variables and functions.
                   //
                   KRC_PACKAGE(PackageLevel2,
                               KRC_UPDATE_THROUGH_GLOBAL_FUNCTION(::updateMyPlugin)
                               KRC_DESCRIPTION("PackageLevel2 description")
                               KRC_GLOBAL(red, ::r,
                   ...
                   KRC_PACKAGE(PackageLevel2,
                               KRC_GLOBAL(green, ::g,
                                   KRC_DESCRIPTION("The green LED color portion")
                                   KRC_UPDATE_THROUGH_GLOBAL_FUNCTION(::someFunc)
                                         )
                               KRC_GLOBAL(blue, ::b,
                                   KRC_DESCRIPTION("The blue LED color portion")
                                   KRC_UPDATE_THROUGH_GLOBAL_FUNCTION(::someFunc)
                                         )
                              )
                  )
